...
import com.liaopeikun.youzhong.core.stateparcel.TrackerStateParcel;
...
public class TrackerListAdapter extends SelectableAdapter<TrackerListAdapter.ViewHolder>
{
    ...
    public void onBindViewHolder(ViewHolder holder, int position)
    {
        ...
        TypedArray a = context.obtainStyledAttributes(new TypedValue().data, new int[] {
                com.liaopeikun.youzhong.R.attr.defaultSelectRect,
                com.liaopeikun.youzhong.R.attr.defaultRectRipple
        });
        ...
        switch (state.status) {
            case TrackerStateParcel.Status.NOT_CONTACTED:
                status = context.getString(com.liaopeikun.youzhong.R.string.tracker_state_not_contacted);
                break;
            case TrackerStateParcel.Status.WORKING:
                status = context.getString(com.liaopeikun.youzhong.R.string.tracker_state_working);
                break;
            case TrackerStateParcel.Status.UPDATING:
                status = context.getString(com.liaopeikun.youzhong.R.string.tracker_state_updating);
                break;
            case TrackerStateParcel.Status.NOT_WORKING:
                if (state.message != null && !TextUtils.isEmpty(state.message)) {
                    status = String.format(context.getString(com.liaopeikun.youzhong.R.string.tracker_state_error), state.message);
                } else {
                    status = context.getString(com.liaopeikun.youzhong.R.string.tracker_state_not_working);
                }
                break;
        }
        ...
        if (state.status == TrackerStateParcel.Status.WORKING) {
            holder.status.setTextColor(ContextCompat.getColor(context, com.liaopeikun.youzhong.R.color.ok));
        } else if (state.status == TrackerStateParcel.Status.NOT_WORKING) {
            holder.status.setTextColor(ContextCompat.getColor(context, com.liaopeikun.youzhong.R.color.error));
        } else {
            holder.status.setTextColor(ContextCompat.getColor(context, com.liaopeikun.youzhong.R.color.text_secondary));
        }
    }
            ...
            implements View.OnClickListener, View.OnLongClickListener
    {
        ...
        public ViewHolder(View itemView, ClickListener listener, List<TrackerStateParcel> states)
        {
            ...
            itemTrackerList = (LinearLayout) itemView.findViewById(com.liaopeikun.youzhong.R.id.item_trackers_list);
            url = (TextView) itemView.findViewById(com.liaopeikun.youzhong.R.id.tracker_url);
            status = (TextView) itemView.findViewById(com.liaopeikun.youzhong.R.id.tracker_status);
        }
